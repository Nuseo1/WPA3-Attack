...
"""
================================================================================
Double_SSID_Attack_Tool  (with 6 GHz Support & Freq Workaround)
================================================================================
...
def select_targets_from_scan(monitor_interface, oui_data, scan_6ghz=False):
    ...
    for target in selected_targets:
        ...
        # 6 GHz question only if the global scan included 6 GHz
        if scan_6ghz:
            ans = input("  [?] Is this a 6 GHz band target? (Wi‑Fi 6E/7/8) [y/N]: ").strip().lower()
            target['is_6ghz'] = (ans == 'y')
        else:
            target['is_6ghz'] = False
...
def run_attack_thread(attack_interface, targets, stop_event, is_multi_ssid, enable_hopping):
    ...
    def update_ap(channel):
        ...
        print(f"[+] [{attack_interface}] Starting {ap_type} AP on CH {channel} | Target: {primary_target['ssid']}")
    ...
    try:
        ...
        kill_proc_robust(hostapd_proc, attack_interface)
        try:
            os.remove(f"/tmp/hostapd_{attack_interface}.conf")
        except Exception:
            pass
...
def main():
    ...
    try:
        ...
        scan_6ghz = input("[?] Include 6 GHz band in scan? [y/N]: ").strip().lower() == 'y'
        if scan_6ghz:
        ...
        if not available_attackers:
            print("\n[!] Only one adapter found. Reusing the scanner adapter for the attack.")
            available_attackers = all_ifaces.copy()
            if enable_hopping:
                print("[!] Channel tracking requires a dedicated second adapter. Disabling channel tracking.")
        ...
        if use_multi_ssid:
            ...
            for i, target in enumerate(targets):
                ...
                if target.get('is_6ghz'):
                    ...
                    print(f"  op_class: {op_cls}  (6 GHz / {target.get('bandwidth_6ghz', 80)} MHz)")
        ...
        # Keep interfaces in their current mode (no cleanup of monitor mode)
        print("[*] Keeping WiFi interfaces in their current mode.")
        cleanup_system()
        print("[+] Done. Goodbye.")
