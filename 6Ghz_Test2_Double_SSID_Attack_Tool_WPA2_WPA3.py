...
def get_airodump_scan_args(include_6ghz=False, as_string=False):
    ...
    # 6 GHz – Channels 1 to 233, step 4 (vollständig inkl. 225, 229, 233)
    # CH 221 → 7055 MHz, CH 225 → 7075 MHz, CH 229 → 7095 MHz (PSC), CH 233 → 7115 MHz
...
def check_interface_6ghz_support(interface):
    try:
        ...
        if phy_match:
            ...
            channels_out = subprocess.check_output(
                ['iw', phy, 'channels'], stderr=subprocess.DEVNULL
            ).decode('utf-8')
...
def prepare_adapter_for_ap(interface):
    if interface.endswith("mon"):
        ...
        try:
            ...
            check = subprocess.run(['ip', 'link', 'show', potential_real_name],
...
def parse_airodump_csv(csv_path):
    ...
    try:
        ...
        ap_section_index = next(
            (i for i, line in enumerate(lines) if 'BSSID, First time seen' in line), -1
        )
        if ap_section_index == -1:
            return []
        ap_lines = lines[ap_section_index + 1:]
        client_section_index = next(
            (i for i, line in enumerate(ap_lines) if 'Station MAC, First time seen' in line), -1
        )
...
def run_attack_thread(attack_interface, targets, stop_event, is_multi_ssid, enable_hopping):
    ...
    def update_ap(channel):
        ...
        args = ['xterm', '-geometry', '90x25', '-T', f'ATTACK: {attack_interface} (CH {channel})',
                '-e', 'bash', '-c', cmd]
        time.sleep(1)
